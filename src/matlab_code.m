...
image_clusters = CalcEdges(RefMag,RefTheta,0.004);
...
MinCluster = 4;
FoundSegs   = Segmenter(image_clusters,RefTheta,image_gray);
...
for i = 1:length(ParentSeg.LSeg)
    ...
    Quad = QuadSearch(Path,LinkedSegs(ParentSeg.LSeg(i)),Depth+1,SegmentList,LinkedSegs);
end
    Quad = [];
end
